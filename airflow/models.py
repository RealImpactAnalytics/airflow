...
class BaseOperator(object):
    ...
    def create_trigger(self, trigger_rule, task):

        if trigger_rule == TriggerRule.ALL_DONE:
            return DoneTrigger(task)

        elif trigger_rule in [TriggerRule.ALL_SUCCESS, TriggerRule.ONE_SUCCESS]:
            return Trigger(task, state=State.SUCCESS)

        elif trigger_rule in [TriggerRule.ALL_FAILED, TriggerRule.ONE_FAILED]:
            return Trigger(task, state=State.FAILED)
    ...
    def set_downstream(self, task_or_task_list):
        ...
        for task in task_list:
            trigger = self.create_trigger(task.trigger_rule, self)
            task.add_trigger(trigger)
            self.add_downstream_task(task)
    ...
    def set_upstream(self, task_or_task_list):
        ...
        for task in task_list:
            trigger = self.create_trigger(self.trigger_rule, task)
...
class Trigger(object):
    ...
    * Trigger(t, [x, y]) depends on the state of t from the now-x schedule to
    ...
    def _states(self, execution_date, main_session=None):
        ...
        states = [row[0] for row in results.all()]
